...
class GroupRec():
    ...
    def predictionBaseline(self, dfProfile, groupTrain, groupTest, gUsers):
        ...
        sim_neighborsDF = pa.DataFrame.from_dict(sim_neighbors, orient='index').reset_index().rename(
            columns={'index': 'userId', 0: 'sim'})
        groupTestItems = groupTest.drop(['userId', 'rating'], axis=1).to_records()
        trainWithoutGroup = self.train[~self.train['userId'].isin(gUsers)]
        trainWithoutGroup = pa.merge(trainWithoutGroup, sim_neighborsDF, left_on=['userId'], right_on=['userId'],
                                     how='left').dropna()
        f = {'rating': ['mean']}
    ...
    def Fmeasure(self, group, groupTrain, groupTest, recommended, Fthreshold):
        ...
        df['TP'] = df.apply(lambda row: 1 if row['rating'] >= Fthreshold and row['ratingRec'] >= Fthreshold else 0,
                            axis=1)
        df['FN'] = df.apply(lambda row: 1 if row['rating'] >= Fthreshold and row['ratingRec'] < Fthreshold else 0,
                            axis=1)
        df['FP'] = df.apply(lambda row: 1 if row['rating'] < Fthreshold and row['ratingRec'] >= Fthreshold else 0,
                            axis=1)
...
def DCGandIDCG(groupTestUser, recommendedDF, dcg):
    ...
    df['pos'] = df.index
    #    print(df)
    df['dcg_item'] = df.apply(
        lambda row: row['rating'] if row['pos'] + 1 == 1 else row['rating'] / (math.log2(row['pos'] + 1)), axis=1)
    #    print(df)
...
def writeToFile(new, text, groupSize):
    if new == 0:
        ...
        file.write(
            'group,NDCG: profile, LM, AVG, AVGLM, Fmeasure: profile, LM, AVG, AVGLM,profileIMP_NDCG,  profileIMP_Fmeasure, ' + str(
...
def main():
    ...
    for i in range(0, length):
        ...
        predictionsIMP = gr.predictAllRatings(groupProfileIMP, sim_neighborsIMP, totalSimBetweenNeighborsIMP,
                                              groupItems)
